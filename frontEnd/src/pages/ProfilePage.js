         ...
         <Card sx={{width:'70%', mx:'auto', backgroundColor:alpha(theme.palette.primary.main, 0.24)}}>
            ...
            <CardContent sx={{fontSize:14}}>
                ...
                <Tooltip title='change profile picture' placement='right'>
                    <Avatar key={avatarKey} alt="ABC" src={auth?.image?`${process.env.REACT_APP_API_BASE_URL}/avatar/${auth.image}?${new Date().getTime()}`:`${process.env.REACT_APP_API_BASE_URL}/avatar/default.png`} sx={{width:'20%', height:'20%', mx:'auto', cursor: 'pointer'}} onClick={handleAvatarClick} />
                </Tooltip>
